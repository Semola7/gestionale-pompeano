...
import { aggiornaScadenzaPersonale, eliminaScadenzaPersonale } from "../actions";
import type { ScadenzaPersonale } from "@/lib/types";

export function ScadenzaRow({ scadenza, personaleId }: { scadenza: ScadenzaPersonale; personaleId: string }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        await aggiornaScadenzaPersonale(scadenza.id, personaleId, formData);
        setInModifica(false);
      } catch (err) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-md border border-zinc-100 px-3 py-2 dark:border-zinc-900">
      ...
      <div className="flex items-center gap-3 text-sm">
        ...
        <form action={eliminaScadenzaPersonale.bind(null, scadenza.id, personaleId)}>
